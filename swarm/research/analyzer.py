...
class ContentAnalyzer:
    ...
    async def analyze_sources(
        ...
        high_relevance_count = sum(1 for r in processed_sources if r.relevance_score >= self.config.research.relevance_threshold)
        
        if high_relevance_count < 3 and len(processed_sources) < self.config.research.max_sources:
            console.print(f"[yellow]🔍 Found only {high_relevance_count} high-relevance sources. Consider expanding search.[/yellow]")
    ...
    async def _analyze_source_with_intelligence(self, source: Dict[str, Any], query: str) -> AnalysisResult:
        ...
        needs_enhancement = (
            analysis.relevance_score < self.config.research.relevance_threshold or
            analysis.word_count < self.config.research.min_word_count
        )
    ...
    async def _extract_deep_content(self, source: Dict[str, Any]) -> Optional[str]:
    ...
    async def _analyze_source(self, source: Dict[str, Any], query: str, method: str = "normal") -> AnalysisResult:
        ...
        if method == "enhanced":
            prompt_template = self.language_helper.get_prompt("enhanced_summary", query=query, title=title, content=content)
        else:
            prompt_template = self.language_helper.get_prompt("source_summary", query=query, title=title, content=content)
    ...
    async def _extract_themes(self, content: str, query: str) -> List[str]:
        ...
        meaningful_words = [
            ...
            if len(word) > 4 and word not in ['that', 'this', 'with', 'from', 'they', 'have', 'will', 'been', 'were', 'said', 'what', 'when', 'where', 'would', 'could', 'should', 'about', 'which', 'their', 'there', 'these', 'those']
        ]
